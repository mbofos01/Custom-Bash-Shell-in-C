#include "library.h"
 /**
  * SOURCE: This class was found on github and was changed to fit our goals,
  * it basically runs a pipe command.
  * 
  * EXAMPLE: cat file | wc -l | wc -c
  * 
  * @see https://gist.github.com/parse/966049
  * 
  * */
...
static int runP(char* cmd, int input, int first, int last);
...
void  runPipe(char *argv)
{
		...
		while (next != NULL) {
			/* 'next' points to '|' */
			*next = '\0';
			input = runP(cmd, input, first, 0);
 
			cmd = next + 1;
			next = strchr(cmd, '|'); /* Find next '|' */
			first = 0;
		}
		input = runP(cmd, input, first, 1);
		cleanup(n);
		n = 0;
